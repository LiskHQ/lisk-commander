    ...
    .action((userInput) => {
      ...
      if (shouldUseJsonOutput) {
        return Promise.all(calls).then((result) => {
          result.map((executed) => {
            if (executed.error) {
              return vorpal.log(JSON.stringify(executed));
            }
            return vorpal.log(JSON.stringify(executed[switchType(userInput.type)]));
          });
      ...
      return Promise.all(calls).then((result) => {
        result.map((executed) => {
          if (executed.error) {
            return vorpal.log(tablify(executed).toString());
          }
          return vorpal.log(tablify(executed[switchType(userInput.type)]).toString());
        });
